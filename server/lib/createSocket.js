...
module.exports = function createSocket(options, bmexClient) {
  ...
  wsClient.onerror = function(e) {
    ...
    try {
      bmexClient.emit('error', e);
    } catch (error) {
      console.log("error on onerror function createSocket",error);
  ...
  wsClient.onend = function(code) {
    ...
    try {
       bmexClient.emit('end', code);
    } catch (error) {
      console.log('WebSocket closed. Please check errors above.');
    }
  };
